        ...
        private int MinMax(ChessBoard chessBoard, SearchMode searchMode, ChessBoard.PlayerColorE ePlayerColor, int iDepth, int iWhiteMoveCount, int iBlackMoveCount, ref int iPermCount) {
            ...
            if (chessBoard.IsEnoughPieceForCheckMate()) {
                if (iDepth == 0 || m_bCancelSearch) {
                    iRetVal = (ePlayerColor == ChessBoard.PlayerColorE.Black) ? -chessBoard.Points(searchMode, ePlayerColor, 0, iWhiteMoveCount - iBlackMoveCount, ChessBoard.m_posInfoNull, ChessBoard.m_posInfoNull) :
                                                                                 chessBoard.Points(searchMode, ePlayerColor, 0, iWhiteMoveCount - iBlackMoveCount, ChessBoard.m_posInfoNull, ChessBoard.m_posInfoNull);
                    iPermCount++;
                } else {
